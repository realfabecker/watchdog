...
async fn notify(notify_url: &String, notify_token: &String, template: String) -> Result<(), Box<dyn error::Error>> {
    ...
    match req.send().await {
        Ok(r) => {
            ...
            Err(Box::new(std::io::Error::new(io::ErrorKind::Other, format!("notify: response err: {}: {}", notify_url, r.status().to_string()))))
        }
        Err(_e) => {
            Err(Box::new(std::io::Error::new(io::ErrorKind::Other, format!("notify: request err: {}: {}", notify_url, _e.to_string()))))
        }
    }
}

fn create_message_from(tmpl: Template) -> String {
    let mut message = tmpl.tmpl
    ...
    return tmpl.body.replace("{{message}}", &message.replace("\n", "\\n"));
}
...
fn get_changes(diff: &str, pattern: String) -> Result<Vec<String>, Box<dyn error::Error>> {
    ...
    let files: Result<Vec<_>, _> = diff.lines()
...
fn git_diff(from: &String, to: &String) -> Result<String, std::io::Error> {
    let output = std::process::Command::new("git")
        ...
        .output()
        .map_err(|_err| std::io::Error::new(io::ErrorKind::Other, format!("unable to run git diff: {}", _err.to_string())))?;

    let stdout = String::from_utf8(output.stdout)
        .map_err(|_err| std::io::Error::new(io::ErrorKind::Other, format!("unable to parse output: {}", _err.to_string())))?;
...
fn read_config(config: String) -> Result<Config, Box<dyn error::Error>> {
    let contents = std::fs::read_to_string(config)
        .map_err(|_err| Box::new(io::Error::new(io::ErrorKind::Other, format!("unable to read yaml: {}", _err.to_string()))))?
    ...
    let config: Config = serde_yaml::from_str(&contents)
        .map_err(|_err| Box::new(io::Error::new(io::ErrorKind::Other, format!("unable to parse yaml: {}", _err.to_string()))))?;
...
async fn sirene(args: Args) -> Result<(), Box<dyn error::Error>> {
    let config = read_config(args.config)
        .map_err(|_err| Box::new(io::Error::new(io::ErrorKind::Other, format!("unable to get config: {}", _err.to_string()))))?;
    ...
    if changes.len() == 0 {
        println!("{}", "no files matching the pattern: ".to_owned() + &config.watches.file_pattern);
        return Ok(());
    }
